...
//   // ✅ Restore data safely from location state or sessionStorage
//   const getState = () => {
...
//     } catch {
//       console.warn("Invalid session storage data, clearing it.");
//       sessionStorage.removeItem("naf_pft_result");
//       return null;
//     }
//   };
...
//   // ✅ Save state to sessionStorage once on mount
//   useEffect(() => {
//     if (state) {
//       sessionStorage.setItem("naf_pft_result", JSON.stringify(state));
//     }
//   }, []); // only runs on first render

//   // Fallback UI if no result
//   if (!state) {
//     return (
//       <div className="no-result">
//         <p>No result available. Please generate a PFT result first.</p>
...
//         pdf.setTextColor(150, 0, 0);
//         pdf.text("RESTRICTED", pageWidth / 2, 10, { align: "center" });
//         pdf.text("RESTRICTED", pageWidth / 2, pageHeight - 8, { align: "center" });
//       };

//       let renderedHeight = 0;
//       while (renderedHeight < imgHeight) {
//         const sourceHeight = Math.min(pageCanvasHeight, imgHeight - renderedHeight);
//         const tempCanvas = document.createElement("canvas");
//         tempCanvas.width = imgWidth;
//         tempCanvas.height = sourceHeight;
//         const ctx = tempCanvas.getContext("2d");
...
//         );
//         const pageImage = tempCanvas.toDataURL("image/png");
//         pdf.addImage(pageImage, "PNG", marginX, marginY, usableWidth, sourceHeight * ratio);
//         addHeaderFooter();
//         renderedHeight += sourceHeight;
...
//   const sendEmail = async () => {
//     try {
//       const response = await fetch("https://naf-pft-sys.onrender.com/send-report", {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ email: state.email, report_data: state }),
//       });
