      ...
      <table className="data-table">
        <thead>
          <tr>
            ...
            <th>Certificates Issued</th>  {/* ← NEW COLUMN */}
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {admins.map((a) => (
            <tr key={a.id}>
              ...
              <td>{a.rank}</td>
              <td>{a.svc_no}</td>
              {/* ← NEW: Certificates Issued Count with Badge */}
              <td>
                <span
                  className={`badge ${
                    a.certificates_count > 0 ? "active" : "zero"
                  }`}
                >
                  {a.certificates_count || 0}
                </span>
              </td>
              <td className="actions">
                {/* ← NEW: View Button */}
                <button onClick={() => viewDetails(a.id)} className="view-btn">
                  View
                </button>
                <button
