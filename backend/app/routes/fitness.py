...
async def update_pft_result(
    ...
    update_data: PFTUpdate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)  # ← login required (you can later restrict to admins)
):
    stmt = select(PFTResult).where(PFTResult.id == result_id)
    result = db.execute(stmt)
    record = result.scalars().first()
...
async def delete_pft_result(
    result_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)  # ← login required (later restrict to admins)
):
    stmt = select(PFTResult).where(PFTResult.id == result_id)
    record = db.execute(stmt).scalars().first()
