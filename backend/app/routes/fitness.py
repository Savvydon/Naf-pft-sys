...
# ==================== HELPER: Check if admin can access a result ====================
def can_admin_access_result(admin_user: User, result: PFTResult, db: Session) -> bool:
    """
    Admin can access a result if:
    1. The result was explicitly tagged with this admin's ID at creation time (NEW)
    2. The admin is a super_admin (handled separately)
    3. Legacy fallback: evaluator who created it is currently assigned to this admin
       ONLY if the record has no admin_id (legacy records).
    """
    if admin_user.role == "super_admin":
        return True

    # NEW: Primary check - result was created when evaluator was under this admin
    if result.admin_id == admin_user.id:
        return True

    # Fallback: Only for legacy records without admin_id
    if result.admin_id is None:
        ...
        # Fallback: match by name/rank for legacy records without evaluator_id
        evaluator = db.query(User).filter(
...
# GET ALL RESULTS - ADMIN ONLY (scoped to assigned evaluators)
@router.get("/pft-results", response_model=List[dict])
async def get_all_pft_results(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin)
):
    """Admin: Get PFT results from assigned evaluators only. Super Admin gets all."""
    try:
        if current_user.role == "super_admin":
            stmt = select(PFTResult).order_by(PFTResult.created_at.desc())
        else:
            # NEW: Get records where admin_id matches this admin
            admin_filter = PFTResult.admin_id == current_user.id

            # Legacy fallback: evaluators currently assigned to this admin
            assigned_evaluator_ids = [
                u.id for u in db.query(User).filter(
                    User.assigned_admin_id == current_user.id,
                    User.role == "evaluator"
                ).all()
            ]

            legacy_filter = and_(
                PFTResult.admin_id.is_(None),
                PFTResult.evaluator_id.in_(assigned_evaluator_ids)
            ) if assigned_evaluator_ids else None

            if legacy_filter is not None:
                stmt = select(PFTResult).where(
                    or_(admin_filter, legacy_filter)
...
# GET RESULT BY ID - ADMIN ONLY (scoped)
...
async def get_pft_result_by_id(
    ...
    """Admin: Get specific PFT result (only if from assigned evaluator)"""
    ...
    # Check permission
    if not can_admin_access_result(current_user, record, db):
        raise HTTPException(
...
# GET RESULTS BY SERVICE NUMBER - ADMIN ONLY (scoped)
...
async def get_pft_results_by_svc_no(
    ...
    """Admin: Search by service number (only within assigned evaluators)"""
    if current_user.role == "super_admin":
        stmt = (
            select(PFTResult)
...
# UPDATE RESULT - ADMIN ONLY (scoped, WITH RECOMPUTATION)
...
async def update_pft_result(
    ...
    update_dict.pop('admin_id', None)  # NEW: Protect admin_id
...
# DELETE RESULT - ADMIN ONLY (scoped)
...
def compute_pft(
    ...
    # New work is stored under the evaluator's current admin. If an evaluator
    # is assigned to an ineligible admin, require the Super Admin to reassign
    # the evaluator before another evaluation can be created. Historical
    # records remain untouched because their admin_id is already stored on the
    # PFTResult row.
    if current_user.assigned_admin_id is not None:
        assigned_admin = db.query(User).filter(
            User.id == current_user.assigned_admin_id,
            User.role == "admin"
        ).first()
        if assigned_admin and not assigned_admin.is_active:
            raise HTTPException(
                status_code=403,
                detail="Your assigned admin is currently ineligible. Contact the Super Admin."
            )
    ...
    # Add evaluator_id to link the record to the evaluator user
    db_data["evaluator_id"] = current_user.id

    # NEW: Capture evaluator's current admin assignment at creation time
    # This ensures past records remain under the previous admin after reassignment
