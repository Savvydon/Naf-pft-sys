...
#     # NEW: Track which evaluator user created this record
#     evaluator_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)
...
#     # NEW: For evaluators — which admin they are assigned to
...
#     # NEW: Evaluator's evaluations
#     evaluations = relationship("PFTResult", back_populates="evaluator_user", foreign_keys="PFTResult.evaluator_id")
#     # NEW: Self-referential for admin-evaluator assignment
...
from sqlalchemy import Column, Integer, String, Float, DateTime, UniqueConstraint, JSON, ForeignKey, Boolean
from sqlalchemy.sql import func
from sqlalchemy.orm import relationship
from .database import Base
...
class User(Base):
    ...
    admin_results = relationship("PFTResult", back_populates="admin_user", foreign_keys="PFTResult.admin_id")
