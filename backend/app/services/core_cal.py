...
def get_component_status(component: str, value: float, cfg: dict, is_time: bool = False) -> dict:
    ...
    elif cfg["type"] == "Closer to ideal":  # BMI logic
        ideal_min, ideal_max = ideal
        result["ideal"] = f"{ideal_min:.1f} - {ideal_max:.1f}"
    ...
    else:  # Lower is better (e.g., cardio)
        result["ideal"] = ideal
        result["excess_time"] = max(0, value - ideal)
...
def compute_naf_pft(data) -> Dict:
    ...
    # --- Strength / Endurance ---
    step_status  = get_component_status("step_up_3min", data.step_up, table["step_up_3min"])
    push_status  = get_component_status("push_up_1min", data.push_up, table["push_up_1min"])
    sit_status   = get_component_status("sit_up_1min", data.sit_up, table["sit_up_1min"])
    chin_status  = get_component_status("chin_up_1min", data.chin_up, table["chin_up_1min"])
    reach_status = get_component_status("sit_reach_cm", data.sit_reach, table["sit_reach_cm"])
